...
function onChangeMenu(menuTarget) {
    ...
    switch (menuTarget) {
        ...
        case "MenuTemplateSession":
            ...
            onChangeDisplay(allDivHomeToDisplayNone,[],["divBtnMenuSimpleReturn","divMenuTemplateSession"],[],[],[],[]);
            onOpenMenuTemplateSession();
            addEventForSimpleReturnMenu(onClickReturnFromMenuTemplateSession);
        break;
        case "NewTemplateSession":
            ...
            onChangeDisplay(["divBtnMenuSimpleReturn","divMenuTemplateSession"],[],["divBtnMenuTriple","divTemplateSessionEditor"],[],[],["btnMenuTripleDelete"],[]);
            onClickBtnCreateTemplateSession();
            addEventForBtnMenuTriple(onClickReturnFromTemplateSessionEditor,onClickDeleteFromTemplateSessionEditor,onClickSaveFromTemplateSessionEditor);
        break;
...
function onLeaveMenu(menuTarget) {
    ...
    switch (menuTarget) {
        ...
        case "MenuTemplateSession":
            ...
            onChangeDisplay(["divMenuTemplateSession","divBtnMenuSimpleReturn"],allDivHomeToDisplayBlock,allDivHomeToDisplayFlex,[],[],[],[]);
        break;
...
let btnReturnEventListener = null;
// Gestion d'évènement pour bouton retour dynamique
function addEventForSimpleReturnMenu(calledFunction) {

    if (devMode === true) {
        console.log("[EVENT-LISTENER] Ajout d'évènement Menu simple avec la fonction :", calledFunction);
    }

    let btnTarget = document.getElementById("btnMenuSimpleReturn");


    // Retire l'ancien évènement s'il existe
    if (btnReturnEventListener && btnTarget) {
        btnTarget.removeEventListener("click", btnReturnEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnReturnEventListener = () => {
        calledFunction();
    };

    // Ajoute l'évènement
    btnTarget.addEventListener("click", btnReturnEventListener);
}




//Div menu double 'retour et valider'

let btnMenuDoubleReturnEventListener = null,
    btnMenuDoubleSaveEventListener = null;

// Gestion d'évènement pour bouton retour dynamique
function addEventForBtnMenuDouble(calledReturnFunction,calledSaveFunction) {

    if (devMode === true) {
        console.log("[EVENT-LISTENER] Ajout d'évènement retour avec la fonction :",calledReturnFunction);
        console.log("[EVENT-LISTENER] Ajout d'évènement save avec la fonction :",calledSaveFunction);
    }

    // bouton retour
    let btnReturnTarget = document.getElementById("btnMenuDoubleReturn");

    // Retire l'ancien évènement s'il existe
    if (btnMenuDoubleReturnEventListener && btnReturnTarget) {
        btnReturnTarget.removeEventListener("click", btnMenuDoubleReturnEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnMenuDoubleReturnEventListener = () => {
        calledReturnFunction();
    };

    // Ajoute l'évènement
    btnReturnTarget.addEventListener("click", btnMenuDoubleReturnEventListener);


    //bouton save
    let btnSaveTarget = document.getElementById("btnMenuDoubleSave");

    // Retire l'ancien évènement s'il existe
    if (btnMenuDoubleSaveEventListener && btnSaveTarget) {
        btnSaveTarget.removeEventListener("click", btnMenuDoubleSaveEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnMenuDoubleSaveEventListener = () => {
        calledSaveFunction();
    };

    // Ajoute l'évènement
    btnSaveTarget.addEventListener("click", btnMenuDoubleSaveEventListener);
}



function removeEventForBtnMenuDouble() {

    if (devMode === true) {
        console.log("[EVENT-LISTENER] Retire les évènements pour menu double");
    }
    ...
    if (btnMenuDoubleReturnEventListener && btnReturnTarget) {
        btnReturnTarget.removeEventListener("click", btnMenuDoubleReturnEventListener);
        btnMenuDoubleReturnEventListener = null;
    }
...
//Div menu triple 'retour supprimer et valider'

let btnMenuTripleReturnEventListener = null,
    btnMenuTripleDeleteEventListener = null,
    btnMenuTripleSaveEventListener = null;

// Gestion d'évènement pour bouton retour dynamique
function addEventForBtnMenuTriple(calledReturnFunction,calledDeleteFunction,calledSaveFunction) {

    if (devMode === true) {
        console.log("[EVENT-LISTENER] Ajout d'évènement retour avec la fonction :",calledReturnFunction);
        console.log("[EVENT-LISTENER] Ajout d'évènement supprimer avec la fonction :",calledDeleteFunction);
        console.log("[EVENT-LISTENER] Ajout d'évènement save avec la fonction :",calledSaveFunction);
    }

    // bouton retour
    let btnReturnTarget = document.getElementById("btnMenuTripleReturn");

    // Retire l'ancien évènement s'il existe
    if (btnMenuTripleReturnEventListener && btnReturnTarget) {
        btnReturnTarget.removeEventListener("click", btnMenuTripleReturnEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnMenuTripleReturnEventListener = () => {
        calledReturnFunction();
    };

    // Ajoute l'évènement
    btnReturnTarget.addEventListener("click", btnMenuTripleReturnEventListener);



    // bouton Supprimer
    let btnDeleteTarget = document.getElementById("btnMenuTripleDelete");

    // Retire l'ancien évènement s'il existe
    if (btnMenuTripleDeleteEventListener && btnDeleteTarget) {
        btnDeleteTarget.removeEventListener("click", btnMenuTripleDeleteEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnMenuTripleDeleteEventListener = () => {
        calledDeleteFunction();
    };

    // Ajoute l'évènement
    btnDeleteTarget.addEventListener("click", btnMenuTripleDeleteEventListener);



    //bouton save
    let btnSaveTarget = document.getElementById("btnMenuTripleSave");

    // Retire l'ancien évènement s'il existe
    if (btnMenuTripleSaveEventListener && btnSaveTarget) {
        btnSaveTarget.removeEventListener("click", btnMenuTripleSaveEventListener);
    }

    // Crée une référence nommée pour la fonction callback
    btnMenuTripleSaveEventListener = () => {
        calledSaveFunction();
    };

    // Ajoute l'évènement
    btnSaveTarget.addEventListener("click", btnMenuTripleSaveEventListener);
}




function removeEventForBtnMenuTripple() {

    if (devMode === true) {
        console.log("[EVENT-LISTENER] Retire les évènements pour menu Triple");
    }
    ...
    if (btnMenuTripleReturnEventListener && btnReturnTarget) {
        btnReturnTarget.removeEventListener("click", btnMenuTripleReturnEventListener);
        btnMenuTripleReturnEventListener = null;
    }
